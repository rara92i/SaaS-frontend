  ...
  const handleAddTrade = async () => {
    if (userData) {
      ...
      const isFreePlanExceeded =
        userData.plan === "free" && newJournalLines > 10;

      if (isTrialExpired || isFreePlanExceeded) {
        alert(
          "Your plan limits have been exceeded. Please upgrade to continue."
        );
        return;
      }

      // Mise à jour du nombre de lignes du journal dans la base de données avant d'ajouter le trade
