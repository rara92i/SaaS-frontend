...
function TradingSimulator() {
  ...
  const handleChange = (e) => {
    ...
    setForm({
      ...form,
      [name]: name === "leverage" && value > 50 ? 50 : value,
    });
  };
  ...
  const simulateTrades = async () => {
    ...
    let outcomes = Array(numWins)
      .fill(true)
      .concat(Array(numLosses).fill(false));
    ...
    await updateUserData(userData.simulations + 1);
  };
  ...
  const BarChart = () => {
    const winCount = tradesData.filter((trade) => trade.outcome === "Win")
      .length;
    const lossCount = tradesData.filter((trade) => trade.outcome === "Loss")
      .length;
      ...
      {tradesData.length > 0 && (
        <>
          <Box mt={4} pt={2} style={{ height: 400, width: "100%" }}>
            <Line
              data={{
                labels: tradesData.map((trade) => trade.tradeNumber),
                datasets: [
                  {
                    label: "Balance",
                    data: tradesData.map((trade) => trade.newBalance),
                  },
                ],
              }}
              options={{
                elements: { line: { tension: 0.5 } },
                plugins: { title: { text: "Account Balance" } },
              }}
            />
          </Box>
          <Box mt={-8} style={{ height: 400, width: "100%" }}>
            <DataGrid
              rows={tradesData}
              columns={columns}
              pageSize={10}
